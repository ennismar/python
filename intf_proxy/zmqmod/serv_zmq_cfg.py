...
from common.settings import INTF_ZMQ_CONN_STR, IOT_SERV_ZMQ_CFG, STREAMER_ZMQ_PULL_STR
from common.iot_msg import *
import zmqmod.handle_msg as handle_msg
...
def init_iot_serv_zmq():
    ...
    socket_pull.bind(INTF_ZMQ_CONN_STR)
    set_zmq_socket_default_option(socket_pull)
    api_serv_stream_pull = zmqstream.ZMQStream(socket_pull)

    api_serv_stream_pull.on_recv(intf_pull_msg)
    logging.info('api server PULL message on: %s', INTF_ZMQ_CONN_STR)

    #streamer pull poll
    streamer_pull = context.socket(zmq.PULL)
    streamer_pull.connect(STREAMER_ZMQ_PULL_STR)
    set_zmq_socket_default_option(streamer_pull)
    streamer_stream_pull = zmqstream.ZMQStream(streamer_pull)
    streamer_stream_pull.on_recv(streamer_pull_msg)
    logging.info('streamer connect to pull message on:%s', STREAMER_ZMQ_PULL_STR)
    # 创建与iot server的连接
    try:
        with open(IOT_SERV_ZMQ_CFG) as config__file:
            ...
            for one_proc in proc_json:
                ...
                if proc_name is None:
                    ...
                    continue
                elif proc_name == PROXY_ZMQ_THREAD_NAME:
                    # 配置进程名与自身相同，无需建立连接
                    continue
                else:
...
def intf_pull_msg(msgs):
    ...
    for one_msg in msgs:
        try:
            ...
            handle_msg.handle_recv_msg(parsed_msg, iot_msg, stream_procs)
        except Exception as e:
            logging.error('Parse msg fail! e = %s', e)
            return

def streamer_pull_msg(msgs):
    for one_msg in msgs:
        try:
            # 解析os 头
            parsed_msg = unpack_rz_msg(one_msg)
