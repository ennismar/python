...
"""
objective: process_manage
for local:
    run command "fab local_run"
    will update the process_manage from svn, then do py_release, and tar as process_manage.tar.gz to commit
    to the svn

for remote:
    run command "fab remote_run"
    will update process_manage.tar.gz on 192.168.1.201:iot111, 192.168.1.201:iot222, 192.168.1.202:iot1, 192.168.1.200:iot
    and replace the process_manage folder except settings.py, and will kill the process if running
...
SOURCE_DIR = 'process_manage'
SOURCE_SVN_DIR = 'process_manage_svn'
RELEASE_DIR = 'serv_release'
TAR_NAME = SOURCE_DIR + '.tar.gz'
EX_INCLUDE_FILES = 'settings.py'
START_FILE = 'start_process_manage.pyc'
